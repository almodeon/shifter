        ...
        for date in all_dates:
            if self.days_filter(date):
                ...
                if staff_count < self.min_staff:
                    violations.append({
                        "date": str(date),
                        "message": f"{date}: {staff_count}/{self.min_staff} {self.shift_type} staff (under minimum)",
                        "constraint_id": self.constraint_id,
                        "severity": self.severity.value,
                        "name": self.name,
                        "type": "STAFFUNDERMIN",
                ...
                if self.max_staff and staff_count > self.max_staff:
                    violations.append({
                        "date": str(date),
                        "message": f"{date}: {staff_count}/{self.max_staff} {self.shift_type} staff (over maximum)",
                        "constraint_id": self.constraint_id,
                        "severity": self.severity.value,
                        "name": self.name,
                        "type": "STAFFOVERMAX",
            ...
            for person_id in scheduler.people.keys():
                for month_key, month_dates in months.items():
                    ...
                    if count > self.limit:
                        violations.append({
                            "date": str(date),
                            "message": f"Person {person_id} in {month_key[1]}/{month_key[0]}: {count} {self.constraint_type} shifts > {self.limit} limit",
                            "constraint_id": self.constraint_id,
                            "severity": self.severity.value,
                            "name": self.name,
                            "type": "ASSIGNSHIFTOVERMAX",
                            "person_id": person_id,
                            "shift_type": self.constraint_type
                        })
            ...
            for person_id in scheduler.people.keys():
                for week_start in weeks:
                    ...
                    if self.min_hours and week_hours < self.min_hours:
                        violations.append({
                            "date": str(date),
                            "message": f"Person {person_id} week {week_start}: {week_hours:.1f}h < {self.min_hours}h minimum",
                            "constraint_id": self.constraint_id,
                            "severity": self.severity.value,
                            "name": self.name,
                            "type": "WKHOURSUNDERMIN",
                            "person_id": person_id,
                            "shift_type": self.constraint_type
                        })
                    if self.max_hours and week_hours > self.max_hours:
                        violations.append({
                            "date": str(date),
                            "message": f"Person {person_id} week {week_start}: {week_hours:.1f}h > {self.max_hours}h maximum",
                            "constraint_id": self.constraint_id,
                            "severity": self.severity.value,
                            "name": self.name,
                            "type": "WKHOURSOVERMAX",
                ...
                for month_key, month_dates in months.items():
                    ...
                    if week_hours_list:
                        ...
                        if self.min_hours and avg_weekly_hours < self.min_hours:
                            violations.append({
                                "date": str(date),
                                "message": f"Person {person_id} {month_key[1]}/{month_key[0]}: avg {avg_weekly_hours:.1f}h/week < {self.min_hours}h minimum",
                                "constraint_id": self.constraint_id,
                                "severity": self.severity.value,
                                "name": self.name,
                                "type": "WKHOURSUNDERMIN",
                        ...
                        if self.max_hours and avg_weekly_hours > self.max_hours:
                            violations.append({
                                "date": str(date),
                                "message": f"Person {person_id} {month_key[1]}/{month_key[0]}: avg {avg_weekly_hours:.1f}h/week > {self.max_hours}h maximum",
                                "constraint_id": self.constraint_id,
                                "severity": self.severity.value,
                                "name": self.name,
                                "type": "WKHOURSOVERMAX",
                ...
                if forbidden and forbidden['date'] in all_dates:
                    ...
                    for forbidden_shift in forbidden['shifts']:
                        if forbidden_shift in assigned_shifts:
                            violations.append({
                                "date": str(date),
                                "message": f"Person {person_id} assigned forbidden {forbidden_shift} on {forbidden['date']}",
                                "constraint_id": self.constraint_id,
                                "severity": self.severity.value,
                                "name": self.name,
                                "type": "FORBIDDENSHIFT",
                                "person_id": person_id,
                                "shift_type": forbidden_shift
                            })
        ...
        for date in all_dates:
            if date in scheduler.holiday_dates:
                ...
                if staff_count < self.required_staff:
                    violations.append({
                        "date": str(date),
                        "message": f"Holiday {date}: {staff_count}/{self.required_staff} staff assigned",
                        "constraint_id": self.constraint_id,
                        "severity": self.severity.value,
                        "name": self.name,
                        "type": "UNDERSTAFFED",
        ...
        for person_id, person in scheduler.people.items():
            for d in all_dates:
                ...
                if not assigned_shifts and not is_vacation and not is_tirocinio and not had_night_before:
                    violations.append({
                        "date": str(d),
                        "message": f"Person {person_id} has no shift on {d}",
                        "constraint_id": self.constraint_id,
                        "severity": self.severity.value,
                        "name": self.name,
                        "type": "NOSHIFT",
